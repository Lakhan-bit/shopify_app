...
app.get("/api/products/all",async(req,res)=>{
  let storeInfo = await shopify.api.rest.Product.all({
...
app.put("/api/product/update",async(req,res)=>{
  ...
  updateProduct.id = req.body.id;
  updateProduct.body_html=req.body.body_html;
...
app.post("/api/product/create",async(req,res)=>{
  let createProduct = new shopify.api.rest.Product({
    session:res.locals.shopify.session,
  });
  createProduct.title = req.body.title;
  createProduct.body_html= req.body.body_html;
  createProduct.vendor = req.body.vendor;
...
app.delete("/api/product/delete",async(req,res)=>{
  ...
  res.status(200).send({Messaege:"delete successfull"});
});
